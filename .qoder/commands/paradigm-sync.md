...
# NEEDED_PATHS for sync (excludes .gitignore — not needed for diff comparison)
NEEDED_PATHS = [
...
> `template_files` 为 `{ rel_path: content }` 内存字典，后续所有阶段使用统一接口。
...
```python
import json
from _template_loader import read_template_version

# Last synced version (from setting.json)
try:
...
if last_synced_version is None:
    print('⚠️  setting.json missing paradigmTemplateVersion, using file diff only')
    last_synced_version = 'unknown'

# Template latest version (from in-memory STATE.md)
template_version = read_template_version(template_files)

print(f'Last synced: {last_synced_version}')
